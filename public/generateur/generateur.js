...
function ajout_attribut(){
  ...
  new_div.innerHTML = '<input id="attribut_'+ index_attributs +'" type = "text"  value =""><span class="spacer"></span><button style="color:red;" onclick="delete_attribut('+index_attributs+')">X</button><br>'
  ...
  index_attributs--;
    for(const child of container.children){      
      if(child.firstChild.id.split('_')[1] > id){
        ...
        child.children[child.children.length - 2].setAttribute('onclick','delete_attribut('+new_id+')');   
      }
    }
...
function ajout_personnage(id){
  ...
  new_div.innerHTML = '<h2 class ="perso" style="font-size:50px;font-weight:bold;text-align:center;" onclick="ajout_personnage('+index_personnages+')" >+</h2>' 
...
if(!document.getElementById('popup_'+id)){
  ...
  let html = '<button style="margin-left:5px;color:red;position:absolute;left:0.5em;bottom:1.3em;" onclick="delete_personnage('+id+')">Supprimer</button>';
  html += '<p class="elements_popup" >Nom : <input type="text" value="'+value_nom+'" id="nom_'+id+'" pattern="[A-Za-z]{3}"> </p> ';
  html += '<p class="elements_popup">Image : <input type="text" value="'+value_img+'" id="image_'+id+'" placeholder="https://..."> </p> ';
  for(const i in attributs){
  ...
  for(const child of container.children){
    if(child.id.split('_')[1] > id){
      ...
      if(typeof child.firstChild.onclick == "function"){
        child.firstChild.setAttribute('onclick','ajout_personnage('+new_id+')');
      }else{
...
function check_validitite_plateau(){
  ...
  for(const p_1 of personnages){
    for(const p_2 of personnages){
      if(p_1.nom == p_2.nom && personnages.indexOf(p_1) != personnages.indexOf(p_2)){
...
function toJson(){
  ...
  if(result_validite == 1){
    ...
    document.getElementById('json_container').innerHTML = '<h2>Json :</h2><button href="#json_container" onclick="toJson()">Générer Json</button><span class="spacer"></span><button onclick="donwload_json()">Télécharger</button><br><br><pre id="json" class="prettyprint linenums"></pre>'
    ...
    switch (result_validite[0]){
      ...
      case -2:
        ...
        break;
      case -3:
        div_json.textContent = "Erreur : Le personnage : "+personnages[result_validite[1]].nom+" a trop d'attributs"
        break;
      case -4:
        div_json.textContent = "Erreur : Le personnage : "+personnages[result_validite[1]].nom+" n'a tous ses attributs définits";
        break;
      case -5:
      let nom_2     
        div_json.textContent = "Erreur : Les personnages "+personnages[result_validite[1]].nom+" et "+personnages[result_validite[2]].nom+" ont les memes valeur d'attributs";
        break;
      case -6:
...
function genereJson(){
  ...
  for(const p of personnages){
    ...
    persos += '{"nom":"'+p.nom+'","visible":"'+p.visible+'","img":"'+p.img+'","attributs":'+atts+'}'
...
function donwload_json(){
  download("Quiestce.json",JSON.stringify(JSON.parse(document.getElementById('json').textContent))) // stringify de parse : remise du json en format compact sans espaces
}
...
function load_json(json){
  ...
  for(const input_attribut of json[0].attributs){
    let new_attribut = new attribut(Object.keys(input_attribut)[0],input_attribut[Object.keys(input_attribut)[0]]);
    attributs.push(new_attribut);
  }
  for(const input_perso of json){
    let new_perso = new personnage(input_perso['nom'],attributs,input_perso['img']);
    for(i in attributs){
      new_perso.attributs[i].valeur = input_perso['attributs'][i][Object.keys(input_perso['attributs'][i])[0]]      
    }
  ...
  for(const attribut of attributs){
    let new_div = document.createElement("div")
  new_div.innerHTML = '<input id="attribut_'+ index_attributs +'" type = "text"  value ="'+attribut.nom+'"><span class="spacer"></span><button style="color:red;" onclick="delete_attribut('+index_attributs+')">X</button><br>'
  new_div.style="margin-top : 10px;"
  document.getElementById("attributs").appendChild(new_div)
  index_attributs++
  }
  ...
  new_div.className = "Personnage";
  new_div.id = "perso_"+index_personnages;
  new_div.innerHTML = '<h2 class ="perso" onclick="ajout_personnage('+index_personnages+')">+</h2>';
  document.getElementById('plateau').appendChild(new_div);
}
